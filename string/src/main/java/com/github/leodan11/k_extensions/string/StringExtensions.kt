...
/**
 * Compresses this [String] using the GZIP format and returns the resulting bytes.
 *
 * This function never throws exceptions. Any error that occurs during
 * encoding or compression is captured and returned as a [Result.failure].
 *
 * ### Usage
 * ```
 * val compressed = text.gzip().getOrNull()
 * ```
 *
 * @receiver String to be compressed.
 * @param charset The [Charset] used to encode the string into bytes
 * before compression. Defaults to [Charsets.UTF_8].
 *
 ...
 * The [Locale] used for case conversion. Default is the system default locale.
 *
 * ```kotlin
 * //example
